...
class ExcelParser(BaseParser):
    ...
    def _build_table_documents(self, df: pd.DataFrame, metadata_base: Dict[str, Any]) -> List[dict]:
        ...
        markdown_full = self._df_to_markdown(df)
    ...
    def _chunk_dataframe(df: pd.DataFrame, max_rows: int = 30) -> List[str]:
        ...
        for start in range(0, len(df), max_rows):
            ...
            chunks.append(ExcelParser._df_to_markdown(chunk_df))
        return chunks
    ...
    def _df_to_markdown(df: pd.DataFrame) -> str:
        """Render a DataFrame to markdown with graceful fallbacks.

        Pandas requires the optional ``tabulate`` dependency for ``to_markdown``. If
        ``tabulate`` is not installed, fall back to CSV formatting; as a last resort
        return a string representation. This keeps parsing resilient in minimal
        environments while still producing structured text for downstream indexing.
        """
